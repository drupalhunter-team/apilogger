...
//Global Variables
HINSTANCE	hIntruder=0, hDDraw=0, hDInput=0, hD3D8=0, hD3D9=0;
...
void install(){
	...
	strcat(szIntruderDir, "intruder.dll");
	LoadLibrary(szIntruderDir);
}
...
extern "C" DWORD __stdcall Direct3DCreate8_(UINT SDKVersion){
	...
	if(hD3D8==0){
		...
		if(hD3D8==0) FatalErrorMsg(TEXT("Can't load d3d8.dll"));
	}
